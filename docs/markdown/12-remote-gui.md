...
| `Esc-{` / `Esc-}` | Cycle through active worlds |
| `Esc-Left` / `Esc-Right` | Cycle through connected worlds |
...
| `Ctrl+P/N` or `Ctrl+Up/Down` | Command history |
...
| `Ctrl+L` | Redraw, keeping only server output |
...
There are no GUI-only menu shortcuts. Use the commands themselves (`/worlds`,
`/worlds -e`, `/setup`, `/disconnect`) or the hamburger menu.
