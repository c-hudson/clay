...
///
/// This is a lookup over `crate::emoji::EMOJI` (moved there in the emoji
/// picker's Step 1 — see `EMOJI-PICKER-ROADMAP.md`), rather than a hand-written
/// match, so the table has exactly one source of truth.
fn emoji_name_to_unicode(name: &str) -> Option<String> {
    let lower = name.to_lowercase();
    crate::emoji::lookup(&lower).map(|s| s.to_string())
}
