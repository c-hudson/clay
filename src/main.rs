...
// TLS support - conditional based on feature flag
...
const TELNET_IAC: u8 = 255;  // Interpret As Command
const TELNET_DONT: u8 = 254;
const TELNET_DO: u8 = 253;
const TELNET_WONT: u8 = 252;
const TELNET_WILL: u8 = 251;
const TELNET_SB: u8 = 250;   // Subnegotiation Begin
const TELNET_GA: u8 = 249;   // Go Ahead (prompt marker)
const TELNET_SE: u8 = 240;   // Subnegotiation End
const TELNET_NOP: u8 = 241;  // No Operation (keepalive)
...
impl WebSocketServer {
    fn new(password: &str, port: u16, allow_list: &str, whitelisted_host: Option<String>) -> Self {
    ...
    fn get_whitelisted_host(&self) -> Option<String> {
        self.whitelisted_host.read().unwrap().clone()
    }

    /// Configure TLS for WSS support
    #[cfg(feature = "native-tls-backend")]
    fn configure_tls(&mut self, cert_file: &str, key_file: &str) -> Result<(), Box<dyn std::error::Error>> {
        use std::fs::File;
        use std::io::Read;
    ...
    fn update_allow_list(&self, allow_list: &str) {
        let allow_list_vec: Vec<String> = allow_list
            .split(',')
            .map(|s| s.trim().to_string())
            .filter(|s| !s.is_empty())
...
fn is_ip_in_allow_list(ip: &str, allow_list: &[String]) -> bool {
    // Normalize localhost
...
/// Get the binary name for use as initial world name
...
impl Theme {
    ...
    fn button_selected_fg(&self) -> Color {
        match self {
    ...
    fn button_selected_bg(&self) -> Color {
        match self {
            Theme::Dark => Color::White,
            Theme::Light => Color::Blue,
        }
...
fn strip_non_sgr_sequences(s: &str) -> String {
...
fn process_telnet(data: &[u8]) -> (Vec<u8>, Vec<u8>, bool, Option<Vec<u8>>) {
...
impl SpellChecker {
    ...
    fn suggestions(&self, word: &str, count: usize) -> Vec<String> {
...
impl InputArea {
    fn new(visible_height: u16) -> Self {
        Self {
            buffer: String::new(),
            cursor_position: 0,
    ...
    fn replace_word(&mut self, start: usize, end: usize, new_word: &str) {
...
struct SpellState {
    suggestions: Vec<String>,  // Includes original word at the end for cycling
...
struct Action {
    name: String,           // Unique name (also used as /name command if no pattern)
    world: String,          // World name to match (empty = all worlds)
    pattern: String,        // Regex pattern to match output (empty = manual /name only)
    command: String,        // Command(s) to execute, semicolon-separated
}
...
/// Calculate how many visual lines a logical line will take when wrapped
...
fn get_current_time_12hr() -> String {
...
/// Strip MUD tags like [channel:] or [channel(player)] from start of line (preserving ANSI)
fn strip_mud_tag(text: &str) -> String {
    // First, find any leading whitespace
    let trimmed = text.trim_start();
    let leading_ws_len = text.len() - trimmed.len();
    let leading_ws = &text[..leading_ws_len];
